...
function historyToggle() {
  $('#historySidebar').addClass('history-opened');
}
function closeButton() {
  $('#historySidebar').removeClass('history-opened');
}
...
function displayCalculation() {
  $('.operator').click(function () {
    ...
    calculationResult = `${operand1} ${operator} ${operand2} = ${result}`;
    addSlogan(calculationResult, operationColors[operator]);
    displayHistory(calculationResult, operationColors[operator]);
  });
}

let hideButton = `<button onclick="$(this).parent().remove()">Hide</button>`;

function displayHistory(calculation, color) {
  historyElement = `<div class="calculation" style="background-color: ${color}; color: #999999">
  <p>${calculation}</p>
  ${hideButton}
  </div>
  `;
  $('#calculationHistory').prepend(historyElement);
}
...
Matter.Engine.update(solver, [(delta = 2)]);
solver.constraintIterations = 6;
solver.velocityIterations = 6;
solver.positionIterations = 12;
...
function addSlogan(calResult = 'hello', color) {
  ...
  // Matter.Events.on(entity, 'click', (e) => {
  //   console.log('clicked');
  // });
...
// for each entity in entities, add evenlistener to remove the entity     TODO
